...
Представление - это простейшая часть структуры MVC - можно сказать, что это набор HTML шаблонов, которые получают переменные из контроллера и которые отсылаются браузеру. Это и есть ваша веб-страница. Обычно представление содержит сниппеты для обработки полученных переменных, такие как например циклы для отображения каждого поста в блоге. Часто их называют шаблонами представлений.

Представления находятся в папке `app/views/controller_name/action_name.html.erb`, где `controller_name` - имя контроллера, с которым связано представление, а `action_name.html.erb` это соответствующий метод в контроллере, выполняющийся непосредственно перед рендерингом представления.
...
*Помните, что необходимо завершать условия и циклы с помощью `<% end %>`! (Определенно, вы несколько раз про это забудете).*
...
*примечание: в режиме "production", ваш CSS и javascripts будет слит в единственный файл, и не волнуйтесь, если его имя будет выглядеть как `/assets/application-485ea683b962efeaa58dd8e32925dadf`*

### Формы

В Rails существует несколько различных помощников (хелперов), создающих формы, и мы ими воспользуемся на предстоящих уроках.
