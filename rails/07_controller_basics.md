...
В каких случаях используется контроллер? Пришедший в приложение HTTP запрос отправляется маршрутизатором в действие контроллера, Rails упаковывает пришедшие с запросом параметры и запускает требуемый метод контроллера. После выполнения метода, Rails перешлет представлению все переменные экземпляра, которые были определены в методе. При этом эти переменные будут "вставлены" в шаблон представления, которое будет отправлено в браузер.
...
В этом простом примере контроллер запрашивает модель ("Эй, дай мне все посты!"), упаковывает их в переменную экземпляра `@posts`, для того, чтобы представление могло их использовать, и затем автоматически рендерит файл представления `app/views/posts/index.html.erb` (мы поговорим об этом через минуту).
...
С одной стороны, Rails немного облегчает вашу жизнь при условии, что все названо определенным образом, а затем выполняется по правилам. Например, ваш контроллер и его действие должны быть названы также, как и в файле`routes.rb`, когда вы направляли не это действие входящий запрос.
...
Возможно вы напишете снипет в представлении для отображения флэш-сообщения, и поэтому вам понадобиться использовать произвольные ключи. Но тем не менее хорошей практикой является использование стандартных ключей. Учтите, что данные хэша флэш-сообщения стираются сразу после его использования, так что не беспокойтесь, что сообщение будет выводиться снова и снова.
