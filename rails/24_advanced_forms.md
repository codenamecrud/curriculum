...
В предыдущих уроках Вы научились создавать простые HTML формы в Rails, теперь, с этими знаниями Вы сможете намного больше. Бывает, что для разработки удобного пользовательского интерфейса нужно создать форму, которая будет обрабатывать множество вещей за раз (например объектов модели). Пользователям нравится кликать кнопку "Отправить", поэтому лучше бы Вам дать им такую возможность. 

В данном разделе мы рассмотрим некоторые способы создания форм с возможностью обработки сразу нескольких объектов модели. Также Вы научитесь предварительно заполнять (prepopulate) выпадающий список(dropdown) объектами.
...
Создание такой формы - многоступенчатый процесс. Нам понадобится контроллер, модель, представление(view) и маршруты(routes)... целая команду MVC! Суть заключается в том, что Ваша форма должна отправить главный объект (например User) как обычно, но данный объект будет включать в себя кучу атрибутов для других объектов, которые Вы хотите создать (например объект(ы) ShippingAddress). Ваша модель тоже должна это учитывать. Это создаст не только исходный объект пользователя, но также вложенные объекты, в то же время. 
...
3. Постройте форму в представлении. Используйте метод `#fields_for` для эффективного создания `#form_for` внутри уже существующей формы  `#form_for`.
...
Обратите внмиания что можно (и нужно) создать новый объект shipping_address в контроллере, а не в представлении; это необходимо для демонстрационных целей.
...
* [Почему не стоит явно вписывать секретный токен в продакшене](http://daniel.fone.net.nz/blog/2013/05/20/a-better-way-to-manage-the-rails-secret-token/)a