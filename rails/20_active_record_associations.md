...
* Как автоматически унисчтожить все объекты-посты пользователя, если этот пользователь был удален?
...
Когда вы создаете ассоциацию, Rails полагается на 2 основных допущения - во-первых, что имя класса модели, на которую указывает ваша ассоциация, основывается непосредственно на имени ассоциации, и, во-вторых, что внешний ключ во всех отношениях `belongs_to` будет назван *имявашейассоциации_id* (например, `user_id`). Как только вы отходите от этих стандартов, вам потребуется указать Rails какой класс ему нужно искать и какой внешний ключ использовать.
...
Но что, если вы хотите иметь 2 типа пользователей, которым принадлежат посты - "автор" и "редактор"? В этом случае, вам потребуется два отдельных внешних ключа в таблице постов, предположим, один названный `author_id` и другой - `editor_id`. Как вы скажете Rails, что каждый из этих внешних ключей в действительности указывает на пользователя (так, чтобы Rails знал - все записи надо искать в таблице пользователей (Users))? Просто укажите класс, на который должна указывать ваша ассоциация, с помощью метко названной опции `:class_name`:
...
Основная суть проста - убедиться, чтобы Rails искал внешний ключ, имеющий соответствующее ассоциации имя, в таблице, также имеющей соответсвующее ассоциации имя. Если что-то из этого не соответствует действительности из-за креативно названных ассоциации или внешнего ключа, вам потрубется это явно указать. Это довольно распространенная задача, чтобы сделать ваши ассоциации более четкими.
...
Так же "commentable" будет использоваться для установки ассоциации. Вам понадобится указать в модели комментарив, что она является полиморфной, чтобы Rails знал о необходимости проверки столбца `commentable_type`, когда ассоциация будет использоваться. Это делается очень просто:
...
Это сохранит пост в базе данных с заполненным внешний ключом пользователя. Грубо говоря, это то же самое, что вызов следующего:
...
* [Пример полиморфной ассоциации](http://terenceponce.github.io/blog/2012/03/02/polymorphic-associations-in-rails-32/)
