...
Вы здесь вероятно для того, чтобы изучать веб-разработку (а иначе… вы видимо ошиблись с выбором места…). Независимо от того, в чем ваша цель – создать собственный веб-сайт или начать карьеру веб-разработчика, самым важным навыком, который вы в итоге получите, является возможность мыслить логически и разбивать проблему на составляющие компоненты. После чего вы сможете обращаться к этим кусочкам по отдельности в разное время. В этом и заключается сущность проектирования и разработки.

Возможно самая важная область при создании веб-сайта, где требуется логическое мышление, это правильное построение вашей модели данных. Данные - это основа почти всех крупных веб-приложений, от простого блога до сложнейшей сети данных Facebook. Невнятная или слишком усложненная модель данных может серьезно помешать, когда вы пытаетесь расти, и даже сделать вашу жизнь как разработчика чрезвычайно мучительной. Если вы работаете с неверными инструментами, что-то «простое» вроде запроса отобразить все комментарии, которые один пользователь оставил к сообщениям другого пользователя, может стоить вам слишком большого количества умственных усилий и циклов ЦПУ.
...
И все-таки что такое Active Record? Вспомните, что Rails – это на самом деле 7 пакетов (модулей? Gems) Ruby, которые гармонично работают вместе. Active Record – это, грубо говоря, пакет, который заботится обо всем, что связано с базами данных. А это и называется “ORM” .
...
Еще более впечатляющим является то, что на самом деле не имеет никакого значения, каким типом базы данных вы пользуетесь (пока вы правильно задаете файл ``config/database.yml``), Active Record сглаживает все различия между базами данных так, что вам уже не надо об э том думать. Вы сосредотачиваетесь на написании кода приложения, а Active Record будет думать о мельчайших подробностях того, как соединить вас и вашу базу данных. Это также обозначает, что если вы переключаетесь с одной базы данных на другую, вам не нужно менять код любого крупного приложения, только некоторые конфигурационные файлы. Звучит логично, не так ли?
...
Если коротко, то Active Record позволяет вам создавать Ruby объект, который представляет собой строку в одной из таблиц базы данных, например ``User``. Создание нового User-a это процесс, состоящий из двух шагов. Во-первых, вам нужно сделать ``User.new``  и возможно передать ему кучу данных (hash) в виде атрибутов: 
...
Это сохранит ваше время, но, как вы увидите позже, иногда вам захочется разделить их в своем приложении.
...
* Думаю, вы уже прочитали [Getting Started with Rails](http://guides.rubyonrails.org/getting_started.html) и сделали упражнение по созданию приложения из вводного раздела Rails Guides. Если еще нет, сделайте это!

* Прочитайте раздел [Active Record Basics](http://guides.rubyonrails.org/active_record_basics.html) Rails Guides. 

* Более подробно Миграции и подтверждения (Migrations and Validations) мы изучим в следующем разделе и в уроке о функциях обратного вызова (Callbacks) позже в этом же курсе.

* Файлы моделей (model files) в Rails находятся в папке ``app/models`` и являются обычными .rb файлами. Ключевой момент здесь это то, что имя класса дается по имени таблицы в вашей базе данных (но в ед.ч.), и этот класс наследует от Active Record::Base и получает его суперсилы.
...
Файл модели, который был создан генератором, это только скелет модельного файла в директории ``app/models`` (вы вполне могли бы создать его сами). Другим ключевым файлом является файл миграции в папке ``db/migrations``, который начинается со сложно выглядящей меткой времени (timestamp) ``20130924230504_create_users.rb``.
...
[*] если только вы не передали Rails генератору названия необходимых колонок, в этом случае они появятся автоматически в полях миграции. Генераторы позволяют [передавать им очень многое в качестве аргументов](http://guides.rubyonrails.org/command_line.html#rails-generate) (pass in arguments).
...
Это серьезные вопросы, и нужно чувствовать себя комфортно, задавая их, даже если вы не слишком уверены в ответах. Если вы не понимаете, о чем я, то вам нужно вернуться назад и прочитать об основах баз данных [в предыдущем уроке](http://www.theodinproject.com/web-development-101/databases).

###Задание###

1. Прочитайте [Migrations chapter of Rails Guides](http://edgeguides.rubyonrails.org/active_record_migrations.html).

    - Не беспокойтесь о пунктах 3.6-3.8

    - Проглядите вскользь секции 7 и 8

    - Сиды (начальные данные, Seeds) полезны и вы будете использовать их позднее. Они сэкономят вам много усилий, особенно когда вы учитесь и в итоге много раз будете выбрасывать базы данных и создавать новые.
...
1. Прочитайте [Rails Guides Validations chapter](http://guides.rubyonrails.org/active_record_validations.html)

    - Секция 2 о вспомогательных функциях (helpers) может быть просмотрена по диагонали – она поможет вам быть более специфичными с валидациями, более подробно вы изучите их позже.
...
Очень скоро вы начнете видеть окружающий мир через призму этих отношений (особенно если не будете делать достаточное количество перерывов). А их истинную силу ощутите тогда, когда необходимо будет их использовать - когда захотите получить данные обо всех объектах, ассоциированных с другим. Хотите посмотреть список всех ваших Twitter последователей? Хотите пересчитать всех своих одноклассников, с которыми живете сейчас в одном городе? Хотите посомтреть все комментарии одного из пользователей, которые он оставил на ленте у другого пользователя? Все эти вещи относительно просты и очевидны, когда вы разберетесь с подходящими отношениями между данными. Поэтому сфокусируйтесь на этих взаимоотношениях.
...
1. Прочитайте начало [гайдлайнов об ассоциациях](http://guides.rubyonrails.org/association_basics.html) до секции 2.7. Остальное мы можем оставить на потом... Важнейшим моментом тут является то, что вы увидите отношения и то, как они работают.
...
Это все придет с практикой,  таким образом, начиная с этого момента проекты будут требовать от вас продумывать модель организации до того, как вы начнете. Несколько минут на предварительные размышления об отношениях играют ключевую роль в выборе правильного направления, когда вы начинаете писать свой код.
