...
Существует несколько тем, которые мы пока не имели возможности изучить, но определенно стоит быть осведомленными об их содержании. В этом разделе мы рассмотрим продвинутый роутинг, темплейты и кратко коснемся метапрограммирования.

## Пункты для размышления

*Постарайтесь ответить на предложенные вопросы. После выполнения задания попробуйте ответить на них ещё раз*

* В каких случаях вам потребуется единичный ресурс (Resource) в роутинге, а когда множественный (Resources)?
* Что такое "не найденный" ("missing") роут при использовании единичного ресурса? (их всего 6, если вы введете команду `$ rake routes`) Что еще не будет хватать в множестве других ресурсов?
* Зачем использовать вложенные роуты?
* В каком порядке вы расположите соответствующие ID ресурсов? Как они будут называться в `params`?
* В каком случае вы можете захотет использовать роут `member`?
* В чем большое сходство роутов `member` и `collection`? В чем их небольшое различие?
* Как вы настроите роут с редиректом, которые так же будет передавать далее любые параметры?
* Как вы будете именовать роутер, используя алиас?
* В каком случае вы можете захотеть использовать вложенные темплейты или несколько темплейтов?
* Как вы (примерно) собираетесь это реализовать?
* Как вы будете передавать переменные между темплейтами?
* Как вы будете передавать (`#yield`) контент в `#content_for`?
* Что такое метапрограммирование?
* Как вы будете использовать метод `#send`, чтобы вызвать другой метод?
* Как вы будете создавать новый метод на лету?
* Когда Ruby вызывает метод `#method_missing`?
* Как вы можете использовать метод `#method_missing` в качестве преимущества?
* Что такое паттерны проектирования?
* Что такое принципы SOLID?

## Продвинутые роуты

Вы должны быть более-менее знакомы с основами роутинга - конвертированием RESTful-запросов, используя знакомые HTTP-глаголы в направление на конкретные экшены контроллеров (используя метод `#resources` или непосредственно задавая их методом `#get`). Это 90% из того, для чего вы обычно будете использовать файл роутов. Но остальные 10% предлагают вам некоторые весьма удобные возможности, такие как редирект прямо из файла роутов, вложенные друг в друга роуты или парсинг параметров из входящего запроса.

### Единичные ресурсы

Вы могли уже встречаться с ними в какой-то момент, не обязательно понимая, что это такое. До текущего момента мы говорили о ресурсах (таких, как "posts" и "users") в ситуациях, когда их было множество. Выглядит довольно интуитивно. В вашем файле `config/routes.rb` вы писали одну простую строку, что-то вроде `resources :users`.

Иногда так же встречаются ситуации, в которых имеет смысл создавать лишь один экземпляр объекта для ресурса. Примером может быть пользовательский личный кабинет, который показывает интересные факты, основанные на том, какой конкретно пользователь в него вошел. Будет лишь один темплейт для кабинета, которые будет показывать информацию, релевантную для вошедшего пользователя.

В этом случае нет необходимости показывать общий список кабинетов, поскольку он всего один (и меняется только в зависимости от пользователя). Мы так же можем сказать, что для любого другого экшена, который обычно потребовал бы ID для идентификации того, с чем мы работаем (вроде `#show`), больше не потребуется параметр `id`.

Строка в файле роутов, обозначающая единичный ресурс будет выглядеть так:
...
...сравним с множественной версией того же самого роута:
