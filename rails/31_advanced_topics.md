...
5. Прочтите раздел 3.4 [Документации по макетам в Rails](http://guides.rubyonrails.org/layouts_and_rendering.html#using-nested-layouts). Здесь речь идет о передаче информации между файлами представлений и макетов, включая стили CSS. Уделите минуту, чтобы понять происходящее в имеющемся там примере.
