...
* Для чего используется `flash` "хэш" used for?
...
Здесь вам на помощь приходит флэш! Используете для сохранения `flash[:success]` (можете назвать как угодно) и эти данные будут доступны вашему представлению до следующего запроса. Как только представление откроет хэщ, Rails сотрет данные, так что они не будут показываться каждый раз, как пользователь переходит на новую страницу. Это очень ловко и удобно.
...
До того, как мы начнем говорить об аутентификации, необходимо расссмотреть фильтры контроллеров. Суть этих фильтров в том, чтобы запускать некий код в какое-то специфическое время, к примеру до того, как отработает любой другой код. Это важно, потому что, к примеру если пользователь хочет запустить действие, на которое у него нет прав, вам нужно пресечь этот запрос на корню и вернуть соответствующую ошибку/редирект, до того, как пользователь совершит любое другое действие. Проще говоря, вы "отфильтровываете" неправомерные запросы.

Мы делаем это, используя фильтр "before", который принимает имя метода, который мы хотим запустить:
...
Лучше прятать методы фильтра в 'private', так чтобы они были доступны только из своего контроллера. 
...
Основная цель аутентификации - это удостовериться в том, что то, кто пытается выполнить какое-либо действие, имеет на это право. Стандартный способ управления этим процессом заключается в том, что пользователь логинится через форму входа. Как только пользователь залогинен, вы отслеживаете его дальнейшие действия с помощью сессий, пока он не выйдет. 

Авторизация - это другая концепция, связанная с аутентификацией. Вы можете быть залогинены, но есть ли у вас права на доступ к тому, к чему вы пытаетесь получить доступ? Банальный пример это разница между обычный пользователем и администратором сайта. Они оба залогинены, но только у админа есть права на изменение определенных вещей. 
...
Во-первых, мы не храним пароли в базе данных в виде текста. Делая так, мы просто напрашиваемся на неприятности (спорим, вы тоже читали очень новости о взломанных сайтах и утекших паролях?). Вместо этого, мы храним зашифрованную версию пароля ("password digest").
...
7. По необходимости создайте методы-хелперы, который позволят вам делать вещи вроде простого определения, залогинен ли пользователь, или сравнить текущего пользователя с другим.
8. Профит.
...
1. Прочтите [Rails Guides on Controllers](http://guides.rubyonrails.org/action_controller_overview.html) главы 5-6.  Сейчас не слишком заморачивайтесь с деталями конфигураций `session_store` в п.5.1.
2. Прочтите [Rails Guides on Controllers](http://guides.rubyonrails.org/action_controller_overview.html#filters) главу 8, чтобы понять фильтры контроллеров.
3. Прочтите [Rails guides on Controllers](http://guides.rubyonrails.org/action_controller_overview.html#http-authentications) главу 11, чтобы больше понять об аутентификации.
...
* [Все о куках (.org)](http://www.allaboutcookies.org/)